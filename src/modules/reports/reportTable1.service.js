...
let Weather = null
try {
	Weather = require('../schema/Weather.model')
} catch {}
...
async function doorNumsOfBuilding(buildingId) {
	const gws = await Gateway.find({ building_id: buildingId })
		.select('_id')
		.lean()
	const gwIds = gws.map(g => g._id)
	if (!gwIds.length) return []
	const nodes = await AngleNode.find({ gateway_id: { $in: gwIds } })
		.select('doorNum')
		.lean()
...
async function getAngleStats({ t0, t1, buildingId, doorNums }) {
	...
	const match =
		dnums && dnums.length ? { ...base, doorNum: { $in: dnums } } : base
...
async function buildTable1Map({ t0, t1, buildingId, doorNums }) {
	...
	if (Weather) {
					...
					stats: [
						{
							$group: {
								...
								h_max: { $max: '$humidity' },
								h_avg: { $avg: '$humidity' },
								w_min: { $min: '$wind_speed' },
								w_max: { $max: '$wind_speed' },
								w_avg: { $avg: '$wind_speed' },
							},
						},
					],
					wind_mode: [
		...
		weather = {
			...
			HUMID_MAX: R(s.h_max),
			HUMID_AVG: R(s.h_avg),
			WINDSPD_MIN: R(s.w_min),
			WINDSPD_MAX: R(s.w_max),
			WINDSPD_AVG: R(s.w_avg),
...
function stripLineSegArray(xml) {
	...
	xml = xml.replace(
		/<(?:\w+:)?lineSegArray\b[^>]*>[\s\S]*?<\/(?:\w+:)?lineSegArray>/gi,
		''
	)
	// <hp:lineSegArray ... />
