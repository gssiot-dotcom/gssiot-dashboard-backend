...
async function buildXYWithNodeContextCsv({
	...
	const gwCtxByDoor = new Map(
		nodes.map(n => {
			...
			return [
				n.doorNum,
				{ serial: gw.serial_number || '', zone: gw.zone_name || '' },
			]
		})
	)
	...
	if (!doorNums.length) {
		...
		const filename = `${fmtDate(startDate)}-${fmtDate(
			endDate
		)}_${safeBuildingName}_${filenameBase}.csv`
	...
	const cursor = AngleNodeHistory.find(
		{
			doorNum: { $in: doorNums },
