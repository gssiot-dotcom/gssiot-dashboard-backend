...
async function gatewayIdsOfBuilding(buildingId) {
	const rows = await Gateway.find({ building_id: O(buildingId) })
		.select('_id')
		.lean()
...
async function doorNumsOfZone({ buildingId, zoneNorm }) {
	...
	const rows = await AngleNode.aggregate([
		...
		{
			$addFields: {
				_pos_norm: { $toLower: { $trim: { input: '$position' } } },
			},
		},
...
function buildRowTokenMap(
	zoneRows,
	maxRows = Number(process.env.REPORT_ZONES_MAX_ROWS || 40)
) {
	const m = {}
